...
import { styles } from '../Results/recordsStyles';
import { ITrainingsReducer } from '../../reducers/trainingsReducer';

interface IOneTraining {
    trainingIndex: number;
}

const OneTraining = (props: IOneTraining) => {
    ...
    const { trainingsList } = useSelector<IState, ITrainingsReducer>((globalState) => ({
        ...globalState.trainings
    }))

    const addData = (name: string, result: number) => {
        const newReference = reference.ref('/' + currentUser?.uid + '/training/' + props.trainingIndex).push();
        newReference
            .set({
                id: newReference.key,
                name: name,
    ...
    const removeData = async (id: string) => {
        await reference.ref('/' + currentUser?.uid + '/training/' + props.trainingIndex + '/' + id).remove();
    };

    const editData = (id: string, name: string, result: number) => {
        reference.ref('/' + currentUser?.uid + '/training/' + props.trainingIndex + '/' + id)
            .update({
                name: name,
                result: result,
            })
            ...
            <View>
                <View style={[styles.recordElement, styles.recordHeader]}>
                    <View style={styles.recordNameResult}>
                        <Text style={styles.recordHeaderText}>{t('Results.Name')}{props.trainingIndex}</Text>
                ...
                {trainingsList.length > 0 && (
                    console.log(Object.values(trainingsList)),
                    Object.values(trainingsList[props.trainingIndex]).map((el) => {
                        return (
                            <View style={styles.recordElement}>
                                <View style={styles.recordNameResult}>
                                    <Text style={styles.recordText}>{el?.name}</Text>
                                    <Text style={styles.recordText}>{el?.result} cm</Text>
                                </View>
                                <View style={styles.recordButtons}>

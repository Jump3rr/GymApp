...
import { IBodyMeasurementsReducer } from '../../reducers/bodyMeasurementsReducer';
...
const BodyMeasurements = () => {
    ...
    const { bodyMeasurementsList } = useSelector<IState, IBodyMeasurementsReducer>((globalState) => ({
        ...globalState.bodyMeasurements
    }))

    const addData = (name: string, result: number) => {
        const newReference = reference.ref('/' + currentUser?.uid + '/body_measurements').push();
        newReference
            .set({
                id: newReference.key,
                name: name,
    ...
    const removeData = async (id: string) => {
        await reference.ref('/' + currentUser?.uid + '/body_measurements/' + id).remove();
    };

    const editData = (id: string, name: string, result: number) => {
        reference.ref('/' + currentUser?.uid + '/body_measurements/' + id)
            .update({
                name: name,
                result: result,
            })
    ...
    return (
        <View>
            ...
            <View>
                ...
                {bodyMeasurementsList.length > 0 && (
                    bodyMeasurementsList.map((el) => {
                        return (
